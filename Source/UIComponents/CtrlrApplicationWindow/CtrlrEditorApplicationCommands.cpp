...
void CtrlrEditor::getAllCommands (Array< CommandID > &commands)
{
	const CommandID ids[] = {	doSaveState,
								...
								doExportFileInstanceVst3,
								doExportFileInstanceRestrictedVst3,
								doExportGenerateUID,
                                 doSearchForProperty,
...
void CtrlrEditor::getCommandInfo (CommandID commandID, ApplicationCommandInfo &result)
{
	...
	switch (commandID)
	{
		...
		case doExportFileInstanceVst3:
			result.setInfo("Export VST3", "Exports the panel as a VST3 instance", panelCategory, 0);
			result.setActive(isPanelActive(true));
			break;

		case doExportFileInstanceRestrictedVst3:
			result.setInfo("Export restricted VST3", "Exports the panel as a restricted VST3 instance. Non editable by user.", panelCategory, 0);
			result.setActive(isPanelActive(true));
			break;
