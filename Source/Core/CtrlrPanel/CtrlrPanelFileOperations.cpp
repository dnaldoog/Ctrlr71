...
const File CtrlrPanel::savePanelAs(const CommandID saveOption)
{
	...
	/***********************************VST3 CODE********************************************************/
	if (saveOption == CtrlrEditor::doExportFileInstanceVst3)
	{
		Result res = owner.getNativeObject().exportWithDefaultPanel(this, false, false);
		if (res.failed())
		{
			AlertWindow::showMessageBox(AlertWindow::WarningIcon, "Panel export", "Failed to export panel as VST3.\n" + res.getErrorMessage());
		}
		else
		{
			AlertWindow::showMessageBox(AlertWindow::InfoIcon, "Panel export", "Wrote new VST3 panel instance");
		}
	}
	if (saveOption == CtrlrEditor::doExportFileInstanceRestrictedVst3)
	{
		Result res = owner.getNativeObject().exportWithDefaultPanel(this, true, true);

		if (res.failed())
		{
			AlertWindow::showMessageBox(AlertWindow::WarningIcon, "Panel export", "Failed to export panel as VST3 instance.\n" + res.getErrorMessage());
		}
		else
		{
			AlertWindow::showMessageBox(AlertWindow::InfoIcon, "Panel export", "Wrote new VST3 instance");
		}
	}
	/***********************************VST3 CODE********************************************************/
